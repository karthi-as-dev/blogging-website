...
import { Image, Tag, X, Loader, Eye, FileText } from 'lucide-react';
...
export default function CreateBlogPage() {
  ...
  return (
    <>
      <Helmet><title>Write a Blog – Inkwell</title></Helmet>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Write a Blog</h1>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Share your knowledge with the world</p>
          </div>
        ...
        <div className="space-y-6">
            ...
            {coverPreview ? (
              <div className="relative aspect-video rounded-2xl overflow-hidden group">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <label className="btn-primary cursor-pointer"><Image size={16} />Change cover</label>
                  <input type="file" accept="image/*" onChange={handleCoverChange} className="hidden" />
                </div>
              </div>
            ) : (
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl py-12 cursor-pointer hover:border-primary-400 dark:hover:border-primary-600 transition-colors group">
          ...
          <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
            ...
            className="w-full text-3xl md:text-4xl font-black bg-transparent border-none outline-none placeholder-gray-300 dark:placeholder-gray-700 text-gray-900 dark:text-white resize-none" />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 p-2 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800 min-h-[44px]">
                ...
                <input value={tagInput} onChange={e => setTagInput(e.target.value)}
                  ...
                  placeholder={form.tags.length < 10 ? 'Add tag...' : 'Max 10 tags'}
                  disabled={form.tags.length >= 10}
                  className="flex-1 min-w-[80px] bg-transparent outline-none text-xs text-gray-700 dark:text-gray-300 placeholder-gray-400" />
              </div>
            </div>
          </div>
